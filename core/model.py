...
class Encoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.hidden3 = nn.Sequential(nn.Conv2d(in_channels=256,
                                     ...
                                     nn.BatchNorm2d(512),
                                     nn.ReLU(True))
        self.hidden4 = nn.Sequential(nn.Conv2d(in_channels=512,
                                               out_channels=100,
                                               kernel_size=(4, 4),
                                               stride=(1, 1),
                                               ),
...
class BackgroundStream(nn.Module):
    ...
    def __init__(self):
        ...
        self.hidden0 = nn.Sequential(nn.ConvTranspose2d(in_channels=100,
                                                        out_channels=512,
                                                        kernel_size=(4, 4),
                                                        stride=(1, 1),
                                                        padding=(0, 0)
...
class VideoGen(nn.Module):
    ...
    def forward(self, x):
        ...
        video = self.fg_stream(latent)
        fg = self.video(video)
        mask = self.mask(video)
        background = self.bg_stream(latent.squeeze(dim=3))  # (-1, 3, 64, 64)
